...
class PluginUpdateChecker_1_3 {
    ...
    private $purchase_code = null; // Envato Marketplace Purchase Code
	...
	public function __construct($metadataUrl, $pluginFile, $slug = '', $checkPeriod = 12, $optionName = ''){
        ...
        $this->purchase_code = get_option( 'ab_envato_purchase_code' );
	...
	public function requestInfo($queryArgs = array()){
		...
		$queryArgs['checking_for_updates'] = 1;
		$queryArgs['installed_version'] = ($installedVersion !== null) ? $installedVersion : '';
        $queryArgs['purchase_code'] = $this->purchase_code;
...
class PluginInfo_1_3 {
    ...
    /**
     * Escape response from html and decode json
     *
     * @param $json
     * @return array|mixed
     */
    public static function escapeFromHtmlAndDecodeJSON( $json ) {
        $decoded_json = json_decode( $json );

        if ( !$decoded_json && preg_match( '/{(.*?)}/', $json, $m ) ) {
            $decoded_json = json_decode( $m[ 0 ] . '}' );
        }
	...
	public static function fromJson($json, $triggerErrors = false){
        ...
        $apiResponse = self::escapeFromHtmlAndDecodeJSON( $json );
