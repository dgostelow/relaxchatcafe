        ...
        <div class="ab-columnizer">
            <?php foreach ($time as $date => $hours) : ?>
                <?php foreach ($hours as $object) : ?>
                    <button <?php echo $object->booked ? 'disabled="disabled"' : '' ?> data-staff_id="<?php echo $object->staff_id ?>" data-date="<?php if ( !$object->is_day ) echo $object->clean_date; ?>" class="<?php echo $object->booked ? 'booked' : '' ?> <?php echo $object->is_day ? 'ab-available-day' : 'ab-available-hour ladda-button zoom-in' ?>" value="<?php echo $object->value ?>">
