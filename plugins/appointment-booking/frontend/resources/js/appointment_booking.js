    ...
    $.fn.appointmentBooking = function(options) {
        var $container = this,
            Appointment = {
                ...
                skip_first_step : (
                    options.attributes.ch
                    && options.attributes.cid
                    && options.attributes.hs
                    && options.attributes.sid
                    && options.attributes.he
                    && options.attributes.ha
                ),
                skip_available  : options.attributes.ha,
                skip_service    : options.attributes.ch
                    && options.attributes.cid
                    && options.attributes.hs
                    && options.attributes.sid
                    && options.attributes.he
                ...
                $.ajax({
                    ...
                    success : function (response) {
                        ...
                        if (response.attributes) {
                            options.attributes.sid = response.attributes.sid;
                            options.attributes.eid = response.attributes.eid;
                        }

                        if (options.attributes.ch && options.attributes.cid){
                            $('.ab-category', $container).hide();
                        }

                        if (options.attributes.hs && options.attributes.sid){
                            $('.ab-service', $container).hide();
                        }

                        if (options.attributes.he){
                            $('.ab-employee', $container).hide();
                        }

                        if (options.attributes.ha){
                        ...
                        // Categories
                        if (options.attributes.cid) {
                            $select_category.val(options.attributes.cid);
                            categoriesView.setSelectedId(options.attributes.cid);
                        }

                        // Services
                        if (options.attributes.sid) {
                            $select_service.val(options.attributes.sid);
                            servicesView.setSelectedId(options.attributes.sid);
                        }

                        // Employee
                        if (options.attributes.eid) {
                            $select_employee.val(options.attributes.eid);
                            staffView.setSelectedId(options.attributes.eid);
                        }
                        ...
                        $requested_date_from.pickadate({
                            dateMin: options.date_min || true,
                            clear: false,
                            today: options.today_text,
                        ...
                        var next_step_handler = function(button_type) {
                            var valid = true,
                            ...
                            if (button_type !== 'mobile' && $requested_time_from.val() == $requested_time_to.val()) {
